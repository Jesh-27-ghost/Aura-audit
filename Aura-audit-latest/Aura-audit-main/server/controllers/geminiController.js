...
/**
 * Assess a code challenge submission
 * POST /api/assess/code-challenge
 */
const assessCodeChallenge = async (req, res) => {
    try {
        const { skillName, code, output, suspicionScore } = req.body;
        const suspicion = JSON.parse(suspicionScore || '{}');

        if (!skillName || !code) {
            return res.status(400).json({ message: 'skillName and code are required' });
        }

        const videoPath = req.file ? req.file.path : null;
        
        // Construct a prompt for Gemini to analyze the code and suspicion
        const prompt = `
            Analyze this coding challenge submission for the skill "${skillName}".
            ...
            ANTI-CHEATING DATA:
            - Tab Switches: ${suspicion.tabSwitches || 0}
            - Window Blur Events: ${suspicion.blurEvents || 0}
            - Copy-Paste Events: ${suspicion.copyPastes || 0}
            ...
            Please provide a JSON assessment with the following structure:
            {
                ...
                "dimensions": {
                    "technicalAccuracy": { "score": number, "observation": string },
                    "efficiency": { "score": number, "observation": string },
                    "bestPractices": { "score": number, "observation": string },
                    "problemSolving": { "score": number, "observation": string }
                },
                "strengths": string[],
                "improvements": string[],
                ...
                "integrityFlags": {
                    ...
                    "copyPasteEvidence": string,
                    "isProxyDetected": boolean,
                    "proxyEvidence": string
                }
            }
            
            Note: If Tab Switches or Focus Loss are high, flag as suspicious in integrityFlags.
        `;
        ...
        let assessmentData = JSON.parse(responseText);
        assessmentData = validateAndNormalizeAssessment(assessmentData, skillName);
        ...
        const assessment = store.insert('assessments', {
            ...
            integrityFlags: assessmentData.integrityFlags,
            integrityFlagged: assessmentData.passed === false && assessmentData.overallScore < 70 // Simplified check
        });
        ...
        res.status(201).json({
            message: 'Code challenge assessment complete',
            assessment: { ...assessment, badgeId: badge ? badge.badgeId : null },
            badgeId: badge ? badge.badgeId : null
        });
        ...
        res.status(500).json({ message: 'Code assessment failed', error: error.message });
    }
};

module.exports = { assessSkill, assessCodeChallenge };
